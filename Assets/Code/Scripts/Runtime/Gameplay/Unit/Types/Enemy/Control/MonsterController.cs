...
namespace Com2usGameDev
{
    ...
    public class MonsterController : MonoBehaviour
    {
        ...
        public void BindBehaviourToController()
        {
            ...
            StateNode.Creator<Nodes.Enemy.Idle>.Using(idle).WithTransition(toRoam).WithTransition(toChasing).WithTransition(toNormalAttack).WithAnimation("mob_idle").Accomplish(controller);
            StateNode.Creator<Nodes.Enemy.Chase>.Using(chasing).WithTransition(toIdle).WithTransition(toSprint).WithTransition(toNormalAttack).WithAnimation("mob_walk").Accomplish(controller);
            StateNode.Creator<Nodes.Enemy.NormalAttack>.Using(normalAttack).WithTransition(toIdle).WithTransition(toChasing).WithTransition(toEmpty).WithAnimation("mob_attack").Accomplish(controller);
            StateNode.Creator<Nodes.Enemy.Roam>.Using(roam).WithTransition(roamToIdle).WithTransition(toChasing).WithTransition(toNormalAttack).WithAnimation("mob_walk").Accomplish(controller);
            StateNode.Creator<Nodes.Enemy.Sprint>.Using(sprint).WithTransition(toIdle).WithTransition(toNormalAttack).WithAnimation("mob_run").AccomplishOnly(controller, specials);
        }
    }
}
